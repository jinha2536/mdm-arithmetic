...
def load_corner_cases(filepath, tokenizer, seed=42):
    """Load Game of 24 corner cases as formatted samples.

    These have output='=24' (no equations). We create formatted samples
    with empty answer region so the model generates freely.
    Evaluation is validation-only (no exact match — multiple valid solutions).
    """
    raw = load_jsonl(filepath)
    samples, metas = [], []
    for d in raw:
        # Create a dummy output of correct length for formatting
        # Model will generate freely; we only validate the result
        dummy_output = EOS_CHAR * MAX_ANS_LEN  # all EOS — will be fully masked
        padded = _rainbow_pad('')  # empty output → all rainbow
        sample = f"{d['input']}{SEP_CHAR}{padded}"
        if len(sample) > MAX_SEQ_LEN:
            continue
        meta = {
            'output_len': 0,
...
@torch.no_grad()
def gen_eval_corner(model, tokenizer, corner_samples, corner_metas,
                    max_len, decode_policy='confidence', device=None):
    """Open-ended generation eval for Game of 24.

    No exact match — validates equations + target only.
    Reports: valid_rate, eq_correct_rate, per-sample details.
    """
    if device is None:
        device = DEVICE
    ...
    groups = {}
    for idx, s in enumerate(corner_samples):
    ...
    for pl, indices in groups.items():
        for bstart in range(0, len(indices), 64):
            ...
            B = len(bind)
            batch_s = [corner_samples[i] for i in bind]
            batch_m = [corner_metas[i] for i in bind]
            ...
            for bi in range(B):
                ...
                results.append({
                    ...
                    'input': meta['input_str'],
                    'pred': validation['clean_output'][:50],
                })
...
def run(tag=''):
    ...
    for mask_type in MASK_TYPES:
        ...
        # Game of 24 corner case evaluation
        if corner_samples is not None:
            print(f"\n  Corner case (Game of 24): {mask_type}")
            r24 = gen_eval_corner(model, tokenizer, corner_samples, corner_metas,
                                  max_len, 'confidence', device=DEVICE)
            all_final[f"{mask_type}_game24_confidence"] = r24
            print(f"    valid={r24['valid_rate']:.3f} eqs_ok={r24['eq_correct_rate']:.3f} "
                  f"target={r24['target_rate']:.3f}")
            for ex in r24.get('examples', [])[:3]:
                print(f"      {ex['input']} → {ex['pred']} {'✓' if ex['valid'] else '✗'}")
