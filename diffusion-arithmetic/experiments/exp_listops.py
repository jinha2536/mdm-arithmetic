...
DECODE_POLICIES = ['confidence', 'l2r']   # l2r = oracle (inner→outer)
...
def _evaluate(node):
    ...
    Each trace entry: {result, op, depth, n_args, n_subexpr, child_depths}
    """
    if isinstance(node, int):
        return node, []
    child_results = []
...
PAD_CHAR = 'P'  # also used as EOS within answer

def build_tok():
    return CharTokenizer(list('0123456789XNDS[] ='),
                         {'mask': 'M', 'pad': PAD_CHAR})
...
def _format_sample(tree):
    ...
    if len(trace_str) > MAX_ANS_LEN:
        return None, None
    padded_trace = trace_str + PAD_CHAR * (MAX_ANS_LEN - len(trace_str))
...
def gen_corner_case_test(n, seed, category='linear_chain'):
    ...
    Corner cases:
      ...
      max_branch:   every op has MAX_ARGS sub-expression children
      flat:         depth 1 only (no nesting)
    """
    rng = random.Random(seed)
    data, metas = [], []
    ...
    def _make_linear(depth):
        ...
        return {'op': op, 'args': args, 'depth': MAX_DEPTH - 1 - depth}  # fix depth later
    ...
    def _make_max_branch(depth):
        if depth <= 0:
            return rng.randint(0, 9)
        op = rng.choice(OPS)
        args = [_make_max_branch(depth - 1) for _ in range(MAX_ARGS)]
    ...
    for _ in range(n * 100):
        ...
        if category == 'linear_chain':
            ...
            tree = _fix_depth(_make_max_branch(min(3, MAX_DEPTH - 1)))
        elif category == 'flat':
...
def gen_eval_with_stats(model, tokenizer, test_samples, test_metas, max_len,
    ...
    """Per-sample generation with tree stats + error positions."""
    if device is None:
        device = DEVICE
    mask_id = tokenizer.special_ids['mask']
    ...
    for st in range(0, len(test_samples), 128):
        batch_s = test_samples[st:min(st + 128, len(test_samples))]
        batch_m = test_metas[st:min(st + 128, len(test_metas))]
        B = len(batch_s)
        prefixes = [s.split('=')[0] + '=' for s in batch_s]
...
def simulate_puma_coverage(model, tokenizer, test_samples, test_metas, max_len,
    ...
    for si in range(N):
        ...
        T_pre = len(penc)
        x = torch.tensor(penc + [mask_id] * MAX_ANS_LEN, dtype=torch.long,
...
@torch.no_grad()
def _quick_gen(model, tokenizer, test_samples, max_len, decode_policy,
               n=None, device=None):
    """Quick generation accuracy check."""
    if device is None:
        device = DEVICE
    mask_id = tokenizer.special_ids['mask']
    pad_id = tokenizer.special_ids['pad']
    subset = test_samples[:n] if n else test_samples
    results = []
    for st in range(0, len(subset), 128):
        batch = subset[st:st + 128]
        B = len(batch)
        ...
        gen, _, _ = generate_diffusion(model, pids, MAX_ANS_LEN, mask_id,
                                       policy=decode_policy, greedy=True, device=device)
        pred = gen[:, pm:pm + MAX_ANS_LEN]
        for i in range(B):
            ps = tokenizer.decode(pred[i].cpu().tolist())
            gs = get_answer(batch[i])
            results.append({'correct': ps == gs})
    return {'accuracy': sum(r['correct'] for r in results) / max(len(results), 1),
            'n': len(results)}
...
def train_model(mask_type, tokenizer, train_samples, test_samples, test_metas,
    ...
    def eval_fn(model, it, tg):
        ...
        if it > 0 and it % GEN_EVAL_EVERY == 0:
            r = _quick_gen(model, tokenizer, test_samples, max_len,
    ...
    model, dynamics = train_diffusion(
        ...
        puma_tau=PUMA_TAU,
        puma_k_schedule=k_sched if mask_type == 'puma' else None,
        n_layer=N_LAYER, n_head=N_HEAD, n_embd=N_EMBD,
...
def run(tag=''):
    ...
    for mt in MASK_TYPES:
        ...
        # Corner cases
        for cat in ['linear_chain', 'max_branch', 'flat']:
    ...
    for dp in DECODE_POLICIES:
        for tt in ['standard', 'corner_linear_chain', 'corner_max_branch', 'corner_flat']:
