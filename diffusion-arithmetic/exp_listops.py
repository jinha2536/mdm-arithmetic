...
def gen_depth_ood_test(n, seed, target_depth):
    ...
    for _ in range(n * 200):
        if len(data) >= n:
            break
        tree = _fix_depth(_make_narrow(target_depth - 1))
        s, m = _format_sample(tree)
...
def gen_eval_with_stats(model, tokenizer, test_samples, test_metas, max_len,
    ...
    for st in range(0, len(test_samples), 128):
        ...
        batch_orders = info.get('orders')  # [B, n_steps] absolute positions
        ...
        for i in range(B):
            ...
            # Decode order analysis: convert absolute positions to answer-relative
            decode_order_ans = None
            dag_violations = 0
            dag_total_deps = 0
            if batch_orders is not None:
                # orders are absolute positions; convert to answer-relative
                ans_start = pm  # prefix length for this batch
                raw_order = batch_orders[i].tolist()
                # Map: for each answer position, at which step was it decoded?
                decode_step = {}
                ...
                # Count DAG violations: parent decoded before child
