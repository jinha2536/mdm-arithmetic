"""Sudoku: training and decode analyses."""
...
from core.train_utils import (
    ...
    train_diffusion, puma_k_fixed, puma_k_step, generate_diffusion, DEVICE,
)
...
# Architecture: paper Table 4 — 8L/8H/256D ≈ 6.4M params
N_LAYER = 8; N_HEAD = 8; N_EMBD = 256
DROPOUT = 0.0; POS_ENC = 'absolute'

# Training: paper §D.2 — 300k iters, batch 256
...
# Paper Table 6: K_start=8, K_end=40 for sudoku (ans_len=81 → ~2 tokens/step at K_end).
PUMA_K_START = 8; PUMA_K_END = 40
PUMA_K_STEP = 5; PUMA_K_EVERY = None
# PAPL (Peng et al. 2026): paper §D.3 uses α=5 for sudoku.
PAPL_TAU = 1.0; PAPL_ALPHA = 5.0
SEED = 42
TRAIN_ONLY = False  # set True via --train-only to skip all eval/analysis
# bfloat16 disabled for fidelity (see exp_addition note).
NO_AMP = True
...
def _apply_naked_subsets(cands, unit, size):
    """Find naked pairs (size=2) or triples (size=3) in a unit."""
...
def _apply_hidden_subsets(cands, unit, size):
    """Find hidden pairs/triples in a unit."""
    unsolved = [c for c in unit if POPCOUNT[cands[c]] > 1]
    if len(unsolved) <= size: return False
    elim_any = False
...
def _apply_forcing_chains(cands):
    """Simple forcing chains (depth-1): if assigning value v to cell i leads to contradiction via singles propagation eliminate v."""
    elim_any = False
    for i in range(81):
        pc = POPCOUNT[cands[i]]
...
def _compute_puzzle_meta(puzzle_str, sol_str):
    """Compute metadata for one puzzle."""
...
def train(mask_type, tokenizer, train_data, test_data_dict, max_len,
    ...
    """Train sudoku model using unified train_diffusion."""
    if device is None: device = DEVICE
...
# Solver-confidence disagreement analysis

def analyse_solver_confidence_disagreement(model, tokenizer, test_data,
                                           batch_size=16, device=None):
    """For each blank cell, compute (solver rank, confidence rank) and classify into 4 groups based on whether each rank is in early-half or..."""
    if device is None:
        device = DEVICE
...
def analyse_decode_strategy(model, tokenizer, test_data, max_len,
    ...
    """Uncover the model's solving strategy by correlating confidence decode rank with multiple cell features."""
    if device is None: device = DEVICE
    model.eval()
    mask_id = tokenizer.special_ids['mask']
...
def run(tag=''):
    ...
    sd = {'config': {k: globals()[k] for k in ['DIFFICULTY_DECAY',
           ...
           'MASK_TYPES', 'DECODE_POLICIES', 'PUMA_K', 'PUMA_TAU',
            'PUMA_K_START', 'PUMA_K_END', 'PUMA_K_STEP']},
