...
# Generate a uniform-across-depths test pool, then bucket by tree_depth.
# Uses exp_listops.gen_test_data, the canonical generator (the experiment's
# build_test_suite also calls this for the 'natural' bucket).  Each entry's
# meta carries 'tree_depth', 'trace_len', and 'children_indices', which we
# repack into the shape listops_decode_analysis expects (prompt, gold_trace,
# trace_len, depth, children_indices).
_N_POOL = 2500   # ~500 per depth in uniform mode → plenty to fill 300/bucket
_TEST_SEED = 7042   # fixed across training seeds so variance is over training only
_pool_samples, _pool_metas = ex.gen_test_data(_N_POOL, seed=_TEST_SEED)
_by_depth = {d: [] for d in [1, 2, 3, 4, 5]}
for s, m in zip(_pool_samples, _pool_metas):
    d = m.get('tree_depth', 0)
    if d in _by_depth:
        _by_depth[d].append((s, m))

test_entries = []
for d in [1, 2, 3, 4, 5]:
    bucket = _by_depth[d][:300]
    for s, m in bucket:
        expr, trace_padded = s.split('=', 1)
        tl = m['trace_len']
        test_entries.append({
            'string': s,
            'prompt': expr + '=',
            'gold_trace': list(trace_padded[:tl]),
            'trace_len': tl,
            'depth': d,
            'children_indices': m['children_indices'],
        })
print(f"test set: {len(test_entries)} entries "
      f"(per-depth: {[len(_by_depth[d]) for d in [1, 2, 3, 4, 5]]})")
