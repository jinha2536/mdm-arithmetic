"""Shared training, generation, and evaluation utilities."""
...
DRIVE_BASE = '/path/to/your/drive/results'
...
def mount_drive():
    try:
        ...
        print("Drive unavailable; using ./results/")
        return False
...
def train_diffusion(
    ...
    """Iteration-based diffusion training with EMA."""
    if device is None:
        device = DEVICE
    ...
    if patience is not None and uses_streaming and puma_k_schedule is not None:
        # Don't activate early stopping until K schedule is mostly ramped.
        _final_k = puma_k_schedule(max_iters)
        _target_k = int(_final_k * 0.9)
        _min_es_iter = max_iters * 2 // 3
        ...
        if uses_streaming:
            ...
            with ctx:
                ...
                if m.sum() == 0:
                    continue
                log_probs = F.log_softmax(logits.float(), dim=-1)
                tlp = log_probs.gather(-1, ids.unsqueeze(-1)).squeeze(-1)
                nll = -tlp
                n_masked = m.sum(dim=-1).clamp_min(1).float()
                if mask_type == 'papl':
                    ...
                    weights = base_w * (1.0 + papl_alpha * w_papl)
        ...
        if log_strata and m.sum() > 0:
            with torch.no_grad():
                if uses_streaming:
                    ...
                    sample_strata_b = sample_strata[buf_orig_idx]
                else:
                    tgt_tok = ids[m]
                    row_idx = m.nonzero(as_tuple=True)[0]
                    sample_strata_b = sample_strata[idx]
                token_losses = F.cross_entropy(
                    logits[m].float(), tgt_tok, reduction='none')
                token_strata = sample_strata_b[row_idx]
...
def generate_diffusion(model, prefix_ids, n_tokens, mask_id,
                       ...
                       reasoning_rank=None,
                       device=None):
    """Greedy masked diffusion generation."""
    if device is None:
        device = DEVICE
    model.eval()
    ...
    for t in range(n_tokens):
        ...
        if policy == 'confidence':
            ...
            min_rank = rank_eff.min(dim=-1, keepdim=True).values
...
def simulate_reveal_trajectory(
    ...
    """Run PUMA-style forward process, tracking the stage at which each position is first revealed."""
    if device is None:
        device = DEVICE
    model.eval()
    mask_id = tokenizer.special_ids['mask']
    ...
    for st in range(0, N, batch_size):
        ...
        B = en - st
        ids_b = test_ids[st:en].clone()
        ans_b = ans_starts[st:en]
        bl_b = blank_masks[st:en].clone()

        ap = (ans_b.unsqueeze(1) + _arange).clamp(max=T - 1)
...
def compute_reveal_vs_order_tau(reveal_stage, reasoning_order, blank_masks=None):
    """Per-example rank correlation between reveal order and a canonical reasoning order, with fallback chain (Kendall tau-b -> tau-c -> Spearman)..."""
    import numpy as np
